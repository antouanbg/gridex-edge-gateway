...
void testIdentityRead() {
    gridex::mbus::MbusNode node(
        {7, gridex::mbus::NodeType::Meter, 401, 0x1122334455667788ULL}
    );
    auto frame = request({7, 0x03, 0x00, 0x00, 0x00, 0x0D});
    const auto response = node.processFrame(frame.data(), frame.size());
    assert(response.size() == 31U);
    assert(response[0] == 7U);
    assert(response[1] == 0x03U);
    assert(response[2] == 26U);
...
void testConfigurationWrite() {
    gridex::mbus::MbusNode node(
        {205, gridex::mbus::NodeType::Unconfigured, 0, 42}
    );
    ...
    appendU16(
        body,
        static_cast<std::uint16_t>(gridex::mbus::NodeType::Inverter)
    );
    appendU16(body, 1001);
    ...
    assert(node.type() == gridex::mbus::NodeType::Inverter);
    assert(node.driverId() == 1001U);
...
void testCrcAndAddressRejection() {
    gridex::mbus::MbusNode node(
        {9, gridex::mbus::NodeType::Unconfigured, 0, 1}
    );
