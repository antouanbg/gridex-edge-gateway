...
|---|---|---|---|
| `gridex_rockpie_service` | Continuous node polling, bounded local telemetry journal and private MQTT health/telemetry implemented; physical commissioning pending | Modbus TCP client to BESS/nodes; normalized Modbus TCP listener northbound; local NDJSON state journal; MQTT over TLS outbound | Polls each canonical ESP32 map independently, records snapshots and transitions locally, exposes the normalized listener, and publishes health/node telemetry only after TLS broker provisioning. It has no MQTT command subscription. |
| `gridex_ota_apply` + ESP32 OTA endpoint | Local OTA service implemented and bench-verified; production rollout pending | ROCK Pi outbound multipart upload to ESP32 local TCP `8080` | The ROCK Pi client has no listener. ESP32 accepts only its provisioned ROCK Pi source, per-node token verifier and matching firmware SHA-256. Site Router may reach ROCK Pi only; it must never route OTA traffic directly to ESP32. |
...
| ESP32-EVB node firmware | Read-only firmware is flashed/polled; logical provisioning and map-v5 health/watchdog are code-ready, but not yet flashed in the pilot; driver unconfigured | CAN or isolated RS485 downstream; Ethernet Modbus TCP `1502`; direct MQTT disabled | DHCP plus local node identity provisioning, one compiled driver for one device type/brand/model/revision, a known ROCK Pi source, health/recovery registers and TTL/sequence handling. Broker credentials belong only on ROCK Pi; the current node rejects all device commands. |
...
|---|---|---|---|
| `gridex_rockpie_service` | Постоянният node polling, ограниченият local telemetry journal и private MQTT health/telemetry са имплементирани; physical commissioning предстои | Modbus TCP client към BESS/нодове; нормализиран northbound Modbus TCP listener; local NDJSON state journal; MQTT over TLS outbound | Poll-ва всяка canonical ESP32 карта независимо, записва snapshots и transitions локално, предоставя нормализиран listener и публикува health/node telemetry само след TLS broker provisioning. Няма MQTT command subscription. |
| `gridex_ota_apply` + ESP32 OTA endpoint | Локалната OTA услуга е имплементирана и bench-потвърдена; production rollout предстои | Outbound multipart upload от ROCK Pi към локален TCP `8080` на ESP32 | ROCK Pi client-ът няма listener. ESP32 приема само provision-натия ROCK Pi source, verifier за token на отделния нод и съвпадащ SHA-256 на firmware-а. Site Router може да достига само ROCK Pi; не трябва да route-ва OTA трафик директно към ESP32. |
...
| ESP32-EVB node firmware | Read-only firmware-ът е flash-нат/poll-ван; logical provisioning и map-v5 health/watchdog са code-ready, но още не са flash-нати в пилота; driver-ът е unconfigured | CAN или изолиран RS485 downstream; Ethernet Modbus TCP `1502`; direct MQTT disabled | DHCP плюс local node identity provisioning, един компилиран driver за един device type/brand/model/revision, известен ROCK Pi source, health/recovery регистри и TTL/sequence обработка. Broker credentials са само на ROCK Pi; текущият нод отказва всички команди към устройства. |
